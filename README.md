...
LCKD 보호소 봉사자들은 현장에서 찍은 사진을 카카오톡 단체 채팅방으로 공유 후 
가끔씩 구글 드라이브로 넘기는 작업으로 진행하여 아래와 같은 세 가지의 문제가 있었습니다.
...
- 드라이브 폴더와 DB 자동 동기화 (Drive에 없는 아이 자동 제거 포함)

#### 📖 인앱 이용 가이드
- 노션 외부 링크 대신 앱 내 가이드 페이지로 제공
- 봉사자/관리자 탭 분리, 아코디언 방식으로 각 기능 상세 안내

#### 🔔 요청 현황 공개
- 비로그인 상태에서도 홈 화면에서 대기 중인 요청 건수 확인 가능
- 봉사자가 요청이 전달됐는지 직접 확인 가능
...
**Vercel CDN 캐싱으로 인한 요청 미반영 문제**
- 기존: 봉사자가 DogDrawer에서 새 아이 요청 시 관리자 목록에 나타나지 않음
- 원인: Vercel CDN이 GET `/api/dog-requests` 응답을 캐싱하여 새 데이터가 반영 안 됨
- 해결: API 응답에 `Cache-Control: no-store` 헤더 명시, 클라이언트 fetch에 `cache: 'no-store'` 옵션 추가
